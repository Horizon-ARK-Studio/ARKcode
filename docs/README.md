...
* [Foundational/VSCODE-IDE-IMPLEMENTATION-PLAN.md](Foundational/VSCODE-IDE-IMPLEMENTATION-PLAN.md) -- the `ide` target's own plan: an on-device VS Code IDE (`code-server`'s Code-OSS workbench + a local Node backend), replacing the current `vscode` flavor's remote-`vscode.dev` shell. Phase 0/1 landed in `../android-project/`; see this document's own progress note at the top for exactly what that does and doesn't cover yet.
* [../android-project/](../android-project/) -- the Android Gradle/Kotlin project itself, scaffolded via ARKlight's `alpha`-branch `arklight android scaffold` and hand-edited per `CODE-STYLE.md`. `../android-project/scripts/vendor-code-server.sh` fetches the VS Code workbench at build time rather than it being committed here -- see that script's own header.
